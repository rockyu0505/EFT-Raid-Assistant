...
- 关注配方：可选模块，内置当前版本的 PvE/PvP 制作与商人兑换快照；按产物分类、搜索并勾选具体配方后，查到其材料时会在价格浮窗中提示对应配方。
...
制作和商人兑换配方使用随版本打包的 `data/recipes.json`，不会在玩家局内实时联网下载。需要更新发布快照时，开发者先刷新物品缓存，再运行：

```powershell
python scripts/update_recipe_data.py
```

要使用配方提示，请在 Settings 的“功能”页开启“关注制作/兑换配方”并重启；随后进入“关注配方”页，按产物树或搜索结果勾选具体配方。PvE/PvP 商人兑换条目分别保存，切换价格模式后可选择另一模式的兑换；制作条目在两模式一致时会共用关注状态。
