...
- 局内控制窗：按 `F9` 打开右上方暗色半透明控制窗，实时调整 PvE/PvP、名称语言、提示时长和透明度；只有启用 Gamma 模块后才会显示画面增强区域。
- 局内日志窗：按 `F10` 打开左下角日志栏；显示时不主动抢游戏焦点，点击后可用滚轮或滚动条查看历史，也可拖动标题移动。
- 主题与字体：Settings 的“界面”页可选择浅色（默认）、深色或高对比度主题，并在 9–18pt 之间调整字号；保存后立即生效，配方树字体与行距会同步刷新。局内半透明浮窗固定使用适合游戏画面的暗色样式。
- 关注配方：可选模块，内置当前版本的 PvE/PvP 制作与商人兑换快照；左侧使用与游戏手册一致的产物分类树，右侧按“产物（配方数）→ 英文商人/设施等级与操作 → 逐项材料”展开，工具使用独立勾选列，另显示制作耗时、兑换限购和简洁的本地化任务依赖；所有列宽均可拖动调整。
- 制作/兑换配方提示：查到关注配方的材料时，价格浮窗会在原有价格信息下方增加独立边框区域，分别突出目标产物和“需求：N 个”，并显示商人/设施等级。
- 商人补货提醒：保留 OCR 倒计时和本地提醒功能，识别后自动为勾选商人设置提醒；补货触发后以左侧半透明悬浮窗常驻提示，可用热键隐藏/显示，仍属于实验功能。
- 功能开关：首次启动默认只启用核心查价，其余功能按需开启；之后可在设置中修改并立即生效。禁用的模块不会显示面板、注册热键、启动后台刷新或应用显示滤镜，仍在完善的模块会标注 Beta。
- 画面增强：可从主 Gamma 页面、F9 控制窗或高级调节窗选择具体 Windows 显示输出；列表同时显示显示器编号、物理显示器名称、显卡名称及可用分辨率/位置。点击“检测所选显示器”会以原值回写验证 Gamma Ramp 接口，不改变画面。明确选择显示器后，驱动拒绝 Gamma Ramp 时会直接报错，不会回退到可能影响全部屏幕的兼容滤镜。
- Gamma 预设热键：每个保存的显示方案都可以单独绑定热键，局内可一键切到指定方案；留空则不绑定。
- 离开自动恢复：画面增强启用后，如果前台既不是 Tarkov 也不是本助手，会自动恢复原始画面；打开 F9 调节时不会误触发恢复。
- 全局日志：运行日志固定显示在主界面底部，不随功能面板切换；可拖动分隔线调整高度或一键收起，状态会在下次启动时恢复。
- 数据可诊断：数据页集中显示价格缓存新鲜度、别名、配方与藏身处快照状态，并保留最近一次数据错误；可一键导出有大小上限的诊断包。
- 性能模式：RapidOCR 默认限制为推荐的 2 个 CPU 线程，也可选择低占用 1 线程或快速 4 线程；正常查价全程在内存中传递截图，不保存整屏或重复 PNG，价格模式切换会复用已经加载的内存数据，Tooltip 边框则使用批量像素运算定位。后台任务结束时不再强制执行全量垃圾回收，空闲内存整理会避开 Tarkov 前台运行期间；启动价格更新先使用低流量 ETag 检查并仅在变化时下载，也可选择仅在缓存过期后再检查。
...
不要只复制或单独发送 exe。程序运行需要旁边的 `_internal` 目录；配方、图标、别名和初始价格缓存都在其中。用户自己的 `config.json`、更新后的 `cache` 和 `debug` 会写在 exe 所在目录，不会覆盖内置资源。
...
Settings 的“性能”页可选择 OCR CPU 占用档位。默认“推荐（2 线程）”用于降低局内查价的瞬时多核占用；修改后会在下一次首次使用对应 OCR 引擎时生效。该页面还可以关闭查价分阶段耗时日志。
...
要使用配方提示，请在 Settings 的“功能”页开启“关注制作/兑换配方”，保存后会立即出现对应面板；随后进入“关注配方”页，按游戏手册分类或搜索结果展开产物并勾选具体配方。“已关注总览”可集中取消、批量删除或清空关注，并可自定义查价浮窗中配方区域的边框颜色。PvE/PvP 商人兑换条目分别保存；制作条目在两模式一致时会共用关注状态。
...
正常查价不会持续写入截图文件；只有装备页检测失败、OCR 异常或没有识别到候选物品名时，才会保存当前 Tooltip、物品名和装备页裁剪，供诊断包使用。

最近一次截图和 OCR 调试图保存在 `debug` 目录。反馈问题时可在“文件 → 导出诊断包”或数据页直接生成 zip；其中包含版本、脱敏设置摘要和有大小上限的最近调试文件。
...
项目使用 PyInstaller。推荐运行完整的可重复候选版打包流程：

```powershell
C:\Users\zetia\miniconda3\envs\eft-raid-assistant\python.exe scripts\package_release.py
```

脚本会依次运行编译、测试、源码 GUI 冒烟、PyInstaller、打包后 exe 冒烟、zip 结构校验和 SHA-256 计算。仅手动构建时可运行：
