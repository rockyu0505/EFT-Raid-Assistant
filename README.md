...
- 关注配方：可选模块，内置当前版本的 PvE/PvP 制作与商人兑换快照；左侧使用与游戏手册一致的产物分类树，右侧按“产物（配方数）→ 商人/设施等级与操作 → 逐项材料”展开，工具使用独立勾选列，产出和材料数量使用不同颜色。
...
制作和商人兑换配方使用随版本打包的 `data/recipes.json`，不会在玩家局内实时联网下载。快照同时保存 tarkov.dev 的游戏手册 `handbookCategories` 分类层级和中文名称。需要更新发布快照时，开发者先刷新物品缓存，再运行：
